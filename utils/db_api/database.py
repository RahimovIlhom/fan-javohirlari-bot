...
class Database:
    ...
    async def add_or_update_user(self, tg_id, language, fullname, phone, region, district, school, science, *args, **kwargs):
        conn = await self.connect
        cur = conn.cursor()
        if await self.select_user(tg_id):
            SQL_query = ("update users set language=?, fullname=?, phone_number=?, region=?, district=?, "
                         "school_number=?, science=?, update_time=? where tg_id=?;")
            cur.execute(SQL_query, (language, fullname, phone, region, district, school, science, datetime.datetime.now(), tg_id))
        else:
            SQL_query = ("insert into users (tg_id, language, fullname, phone_number, region, district, school_number, "
                         "science, created_time, update_time) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);")
            cur.execute(SQL_query, (tg_id, language, fullname, phone, region, district, school, science,
